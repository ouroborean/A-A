...
@pytest.fixture
def byakuya_test_scene(test_scene: BattleScene) -> engine.Scene:
    ally_team = [Character("byakuya"), Character("toga"), Character("nemu")]
    enemy_team = [Character("astolfo"), Character("naruto"), Character("toga")]
    test_scene.setup_scene(ally_team, enemy_team)
    return test_scene
...
# def test_imperial_sword_base(byakuya_test_scene: BattleScene):
#     pteam = byakuya_test_scene.player_display.team.character_managers
#     eteam = byakuya_test_scene.enemy_display.team.character_managers

#     byakuya = pteam[0]
#     naruto = eteam[1]

#     imperial = byakuya.source.alt_abilities[1]

#     ally_use_ability(byakuya_test_scene, byakuya, naruto, imperial)

#     assert naruto.source.hp == 70


# def test_imperial_sword_injured(byakuya_test_scene: BattleScene):
#     pteam = byakuya_test_scene.player_display.team.character_managers
#     eteam = byakuya_test_scene.enemy_display.team.character_managers
...
#     byakuya.source.hp = 40

#     ally_use_ability(byakuya_test_scene, byakuya, naruto, imperial)

#     assert naruto.source.hp == 40

# def test_imperial_sword_injured_partial(byakuya_test_scene: BattleScene):
#     pteam = byakuya_test_scene.player_display.team.character_managers
#     eteam = byakuya_test_scene.enemy_display.team.character_managers
...
def test_naruto_mission_4(naruto_test_scene: BattleScene):
    ...
    assert naruto.source.mission4progress == 0
    assert not naruto.source.mission4complete

    for enemy in eteam:
        assert naruto.source.mission4progress == 0
        ally_use_ability(naruto_test_scene, naruto, enemy, toad_taijutsu)

    assert naruto.source.mission4progress == 1
...
def test_naruto_mission_5_success(naruto_test_scene: BattleScene):
    pteam = naruto_test_scene.player_display.team.character_managers
    eteam = naruto_test_scene.enemy_display.team.character_managers
    naruto = pteam[0]
    toga = pteam[1]
    ...
    ally_use_ability(naruto_test_scene, naruto, enemy, rasengan)
    ally_use_ability(naruto_test_scene, naruto, enemy, rasengan)
    ally_use_ability(naruto_test_scene, naruto, enemy, rasengan)
    ally_use_ability(naruto_test_scene, naruto, enemy, rasengan)

    assert enemy.source.dead
    assert naruto.source.mission5progress == 1


def test_naruto_mission_5_failure1(naruto_test_scene: BattleScene):
    pteam = naruto_test_scene.player_display.team.character_managers
    eteam = naruto_test_scene.enemy_display.team.character_managers
    naruto = pteam[0]
    toga = pteam[1]
    ...
    rasengan = naruto.source.main_abilities[1]
    knife = toga.source.main_abilities[0]
    ...
    ally_use_ability(naruto_test_scene, toga, enemy, knife)

    ally_use_ability(naruto_test_scene, naruto, enemy, rasengan)

    assert enemy.source.dead
    assert naruto.source.mission5progress == 0

def test_naruto_mission_5_failure2(naruto_test_scene: BattleScene):
    pteam = naruto_test_scene.player_display.team.character_managers
    eteam = naruto_test_scene.enemy_display.team.character_managers
    naruto = pteam[0]
    toga = pteam[1]
...
def test_itachi_mission_1(itachi_test_scene: BattleScene):
    pteam = itachi_test_scene.player_display.team.character_managers
    eteam = itachi_test_scene.enemy_display.team.character_managers
    itachi = pteam[0]
    amaterasu = itachi.source.main_abilities[0]

    assert not itachi.source.mission1complete

    for enemy in eteam:
        ally_use_ability(itachi_test_scene, itachi, enemy, amaterasu)
    
    assert itachi.source.mission1complete
    assert itachi.source.mission1progress == 1
...
def test_itachi_mission_2(itachi_test_scene: BattleScene):
    pteam = itachi_test_scene.player_display.team.character_managers
    eteam = itachi_test_scene.enemy_display.team.character_managers
    itachi = pteam[0]
    enemy = eteam[0]
    ...
    assert itachi.source.mission2progress == 0

    ally_use_ability(itachi_test_scene, itachi, enemy, tsukuyomi)
...
def test_itachi_mission_3(itachi_test_scene: BattleScene):
    ...
    assert itachi.source.mission3progress == 0

    ally_use_ability(itachi_test_scene, itachi, itachi, susanoo)
    ally_use_ability(itachi_test_scene, itachi, enemy, totsuka)
    ally_use_ability(itachi_test_scene, itachi, enemy, totsuka)
    ally_use_ability(itachi_test_scene, itachi, enemy, totsuka)
    ally_use_ability(itachi_test_scene, itachi, enemy, totsuka)
    ally_use_ability(itachi_test_scene, itachi, enemy, totsuka)

    assert itachi.source.mission3progress == 1

def test_itachi_mission_4(itachi_test_scene: BattleScene):
    pteam = itachi_test_scene.player_display.team.character_managers
    eteam = itachi_test_scene.enemy_display.team.character_managers
    itachi = pteam[0]
    enemy = eteam[1]
    ...
    assert itachi.source.mission1progress == 0
    ...
    assert itachi.source.hp == 10
    assert not itachi.has_effect(EffectType.DEST_DEF, "Susano'o")

    itachi_test_scene.win_game_mission_check()
    
    assert itachi.source.mission4progress == 1


def test_itachi_mission_5(itachi_test_scene: BattleScene):
    pteam = itachi_test_scene.player_display.team.character_managers
    eteam = itachi_test_scene.enemy_display.team.character_managers
    itachi = pteam[0]
    enemy = eteam[1]
    susanoo = itachi.source.main_abilities[2]
    taijutsu = enemy.source.main_abilities[2]

    assert itachi.source.mission5progress == 0

    ally_use_ability(itachi_test_scene, itachi, itachi, susanoo)
    enemy_use_ability(itachi_test_scene, enemy, itachi, taijutsu)
    assert itachi.source.mission5progress == 30
    enemy_use_ability(itachi_test_scene, enemy, itachi, taijutsu)
    assert itachi.source.mission5progress == 45
    
def test_minato_mission_4_success(minato_test_scene: BattleScene):
    pteam = minato_test_scene.player_display.team.character_managers
    eteam = minato_test_scene.enemy_display.team.character_managers
    minato = pteam[0]
    ...
    for enemy in eteam:
        ally_use_ability(minato_test_scene, minato, enemy, kunai)
        assert enemy.source.hp == 90
        assert enemy.has_effect(EffectType.MARK, "Marked Kunai")

    for enemy in eteam:
        ally_use_ability(minato_test_scene, minato, enemy, raijin)
        

    assert minato.source.mission4progress == 1
...
def test_minato_mission_4_success(minato_test_scene: BattleScene):
    ...
    for enemy in eteam:
        ally_use_ability(minato_test_scene, minato, enemy, raijin)
        tick_one_turn(minato_test_scene)
        tick_one_turn(minato_test_scene)
